# brew
# Autogenerated from man page /opt/homebrew/share/man/man1/brew.1
...
complete -c brew -l 1 -d 'Only show dependencies one level down, instead of recursing'
...
complete -c brew -l all -d 'List dependencies for all available formulae'
...
complete -c brew -s 1 -d 'Force output to be one entry per line'
...
complete -c brew -l greedy -d 'Print outdated casks with auto_updates true or version :latest'
complete -c brew -l greedy-latest -d 'Print outdated casks including those with version :latest'
complete -c brew -l greedy-auto-updates -d 'Print outdated casks including those with auto_updates true'
...
complete -c brew -l desc -d 'Search for formulae with a description matching text and casks with a name ma…'
...
complete -c brew -l zap -d 'Remove all files associated with a cask'
complete -c brew -l merge -d 'Use git merge to apply updates (rather than git rebase)'
complete -c brew -l preinstall -d 'Run on auto-updates (e. g.  before brew install).  Skips some slower steps'
...
complete -c brew -l appcast -d 'Audit the appcast'
...
complete -c brew -l display-cop-names -d 'Include the RuboCop cop name for each violation in the output'
complete -c brew -l display-filename -d 'Prefix every line of output with the file or formula name being audited, to m…'
complete -c brew -l display-failures-only -d 'Only display casks that fail the audit.  This is the default for formulae'
...
complete -c brew -l open-pr -d 'Open a pull request for the new version if there are none already open'
...
complete -c brew -l message -d 'Append message to the default pull request message'
...
complete -c brew -l macos -d 'Version(s) of macOS the bottle should be built for'
...
complete -c brew -l fail-if-not-changed -d 'Return a failing status code if no changes are detected in the manpage outputs'
...
complete -c brew -l branch -d 'Branch to publish to (default: master)'
...
complete -c brew -l no-compat -d 'Do not load the compatibility layer when running tests'
