# tcpdump
# Autogenerated from man page /Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk/usr/share/man/man1/tcpdump.1
...
complete -c tcpdump -s c -d 'PD Exit after receiving or displaying count packets'
...
complete -c tcpdump -s g -d 'Do not insert line break after IP header in verbose mode for easier parsing'
...
complete -c tcpdump -l interface -d 'PD Listen on interface'
...
complete -c tcpdump -s k -d 'Control the display of packet metadata via an optional  metadata_arg argument'
...
complete -c tcpdump -s P -d 'Use the pcap-ng file format when saving files.  Apple modification'
complete -c tcpdump -l no-promiscuous-mode -d 'PD Don\'t put the interface into promiscuous mode'
complete -c tcpdump -s Q -d 'To specify a filter expression based on packet metadata information like inte…'
...
complete -c tcpdump -o ttt -d 'Print a delta (micro-second resolution) between current and previous line on …'
complete -c tcpdump -o tttt -d 'Print a timestamp, as hours, minutes, seconds, and fractions of a second sinc…'
complete -c tcpdump -o ttttt -d 'Print a delta (micro-second resolution) between current and first line on eac…'
complete -c tcpdump -l time-zone-offset -d 'To set the time zone where the packet capture file was saved'
complete -c tcpdump -s u -d 'Print undecoded NFS handles'
complete -c tcpdump -l packet-buffered -d 'PD If the  -w option is not specified, make the printed packet output ``packe…'
...
complete -c tcpdump -s X -d 'When parsing and printing, in addition to printing the headers of each packet…'
complete -c tcpdump -o XX -d 'When parsing and printing, in addition to printing the headers of each packet…'
complete -c tcpdump -l linktype -d 'PD Set the data link type to use while capturing packets to datalinktype'
...
complete -c tcpdump -s i -d 'flag to specify an interface on which to capture'
...
complete -c tcpdump -s s
complete -c tcpdump -s y
complete -c tcpdump -s Z
