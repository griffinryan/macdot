...
complete -c pcre2grep -s f -l file -d 'Read patterns from the file, one per line, and match them against each line o…'
...
complete -c pcre2grep -s x -d 'given any number of times.  If a directory matches both --include-dir and'
...
complete -c pcre2grep -l line-regex -l line-regexp -d 'Force the patterns to start matching only at the beginnings of lines, and in …'
