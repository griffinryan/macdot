# make
# Autogenerated from man page /Applications/Xcode.app/Contents/Developer/usr/share/man/man1/make.1
complete -c make -s b -s m -d 'These options are ignored for compatibility with other versions of R make '
...
complete -c make -l file -l makefile -d 'Use  file as a makefile'
...
complete -c make -s n -l just-print -l dry-run -l recon -d 'Print the commands that would be executed, but do not execute them'
...
complete -c make -s f -d 'option is present, make will look for the makefiles GNUmakefile , makefile , …'
